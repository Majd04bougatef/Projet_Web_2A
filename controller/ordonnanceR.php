...
class ordonnancefunction
{
    ...
    {
        $sql = "SELECT user.nom , user.prenom ,user.specialite FROM user,rendezvous WHERE user.id_user = rendezvous.id_med and rendezvous.id_rdv = :id and user.role='medecin'";
        $db = config::getConnexion();
        try {
            $liste = $db->prepare($sql);
            $liste->execute(['id' => $id]);
